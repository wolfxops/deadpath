...
def test_in_file_hypotheses_flag_dormant_not_dead(tmp_path: Path) -> None:
    ...
    _write(tmp_path, "app/kept.py", "# deadpath: keep — loaded by the ops runbook\nVALUE = 1\n")
    result = scan_repo(tmp_path, memory=False)
    found = _by_id(result)
...
def test_legacy_unreach_keep_marker_still_honored(tmp_path: Path) -> None:
    _write(tmp_path, "app/__init__.py", "")
    _write(tmp_path, "app/main.py", "print('ok')\n")
    _write(tmp_path, "app/legacy.py", "# unreach: keep — leftover marker from the previous package name\nVALUE = 1\n")
    result = scan_repo(tmp_path, memory=False)
    critique = _by_id(result)["orphan_file:app/legacy.py"].critique
    assert critique["verdict"] == "keep"
    assert critique["objections"][0]["hypothesis"] == "explicit_keep_marker"
...
def test_judge_cli_table_and_mcp(capsys) -> None:
    ...
    reply = handle_request(
        {"jsonrpc": "2.0", "id": 1, "method": "tools/call", "params": {"name": "deadpath.judge", "arguments": {"path": str(DEADAPP), "memory": False}}},
        session,
    )
    text = reply["result"]["content"][0]["text"]
    assert text.startswith("# Deadpath judge") and "| keep |" in text
